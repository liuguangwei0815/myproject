...
/**
 * OAuth 授权服务器配置 https://segmentfault.com/a/1190000014371789
 * 
 * @author wunaozai
 * @date 2018-05-29
 */
...
public class AuthorizationServerConfiguration extends AuthorizationServerConfigurerAdapter {
	...
	public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
		...
		/*
		 * //配置客户端,一个用于password认证一个用于client认证 clients.inMemory() .withClient("client_1")
		 ...
		 * .resourceIds(DEMO_RESOURCE_ID) .authorizedGrantTypes("password",
		 * "refresh_token") .scopes("select") .authorities("oauth2")
		 * .secret(finalSecret) .and() .withClient("client_code")
		 * .resourceIds(DEMO_RESOURCE_ID) .authorizedGrantTypes("authorization_code",
		 * "client_credentials", "refresh_token", "password", "implicit") .scopes("all")
		 * //.authorities("oauth2") .redirectUris("http://www.baidu.com")
